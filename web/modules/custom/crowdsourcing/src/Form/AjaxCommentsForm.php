...
use Drupal\crowdsourcing\TempStore;
...
class AjaxCommentsForm extends CommentForm {
   ...
   *
   * @var \Drupal\crowdsourcing\FieldSettingsHelper
   */
   ...
   *
   * @var \Drupal\crowdsourcing\TempStore
   */
  protected $tempStore;
   ...
   *   The CurrentRouteMatch service.
   * @param \Drupal\crowdsourcing\FieldSettingsHelper $field_settings_helper
   *   The FieldSettingsHelper service.
   * @param \Drupal\crowdsourcing\TempStore $temp_store
   *   The TempStore service.
   */
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      ...
      $container->get('crowdsourcing.field_settings_helper'),
      $container->get('crowdsourcing.temp_store')
    );
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if ($is_ajax && in_array($route_name, ['crowdsourcing.edit', 'crowdsourcing.reply'])) {
  ...
  protected function actions(array $form, FormStateInterface $form_state) {
    ...
    $ajax = [
      ...
      // override below.
      'url' => NULL,
      // We need to wait for crowdsourcing_entity_display_build_alter() to run
    ...
    switch ($route_name) {
      ...
      case 'crowdsourcing.save':
        $element['submit']['#ajax'] = $ajax;
        // If the user attempted to submit the form but there were errors,
        // rebuild the form used at the 'crowdsourcing.edit' route.
        if ($editing) {
          $element['submit']['#ajax']['url'] = $ajax_edit_comment_url;
          $element['cancel'] = $cancel;
        }
      ...
      case 'crowdsourcing.reply':
        $element['submit']['#ajax'] = $ajax;
        $element['submit']['#ajax']['url'] = $ajax_comment_reply_url;
        $element['cancel'] = $cancel;
        break;

      case 'crowdsourcing.save_reply':
        $element['submit']['#ajax'] = $ajax;
        // If the user attempted to submit the form but there were errors,
        // rebuild the form used at the 'crowdsourcing.reply' route.
        if ($editing) {
          $element['submit']['#ajax']['url'] = $ajax_comment_reply_url;
          $element['cancel'] = $cancel;
        }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->hasAnyErrors() && in_array($route_name, ['crowdsourcing.save', 'crowdsourcing.save_reply'])) {
